...
droid-devtools makes **zero** outbound network calls to third-party servers. There is no telemetry, analytics, tracking, or data exfiltration of any kind.
...
- All session data is read **locally** from `~/.factory/` — it never leaves your machine.
- The app does not write to session files. Volume mounts in Docker use `:ro` (read-only) by default.
- Configuration is stored at `~/.factory/droid-devtools-config.json` on the local filesystem.
...
- File reads are constrained to the project root and `~/.factory/`
